...
object StandardEntity {
  ...
  def apply[A](aup:A => (Int, Int, Cell, TerrainCache, PlayerInfo, Random) => (A, Cell, Seq[GlobalMessage]),
...
trait StandardEntity {
  type T
  val up:T => (Int, Int, Cell, TerrainCache, PlayerInfo, Random) => (T, Cell, Seq[GlobalMessage])
  ...
  def update(pos:Cell, cache:TerrainCache, pi:PlayerInfo, r:Random):(StandardEntity, Cell, Seq[GlobalMessage]) = {
    if (pi.live == Dead) {
