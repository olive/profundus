...
class Engine {
  val rows = 32 + 16
  val cols = 32
  val m = GameMode.create(cols, rows, Loadout.default, 0)//TitleMode.create(cols, rows)//LoadoutMode.create(cols, rows, None)//ResultMode.create(cols, rows, PlayerLog.create(Loadout.default))//StoryMode.create(cols, rows, Loadout.default)//
