...
class Engine {
  val rows = 32 + 16
  val cols = 32
  val m = GameMode.create(cols, rows, Loadout.default)//LoadoutMode.create(cols, rows, None)//TitleMode.create(cols, rows)//ResultMode.create(cols, rows, PlayerLog.create(Loadout.default))//StoryMode.create(cols, rows, Loadout.default)//
