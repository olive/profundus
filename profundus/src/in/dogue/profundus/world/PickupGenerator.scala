    ...
    def gen(cols:Int, rows:Int, yRoom:Int, cache:Array2d[WorldTile], ts:TerrainScheme, r:Random) = {
      val foods = FoodType.All(r).map { ft =>
        (0 until 10).map { i =>
          ...
          FoodPickup.create((x, y + yRoom), ft).toPickup.onlyIf(cache.get((x, y)).isWalkable)
        }.flatten
      }
...
case class PickupGenerator(private val f:(Int,Int,Int,Array2d[WorldTile], TerrainScheme, Random) => Seq[Pickup]) {
  def generate(cols:Int, rows:Int, y:Int, cache:Array2d[WorldTile], ts:TerrainScheme, r:Random):Seq[GlobalSpawn] = {
    import Profundus._
